...
public sealed class BackupRestoreVerificationTests : IDisposable
{
    ...
    private static async Task SeedLedgerAsync(ServiceHubDbContext dbContext, string ownerId, int entryCount)
    {
    ...
    public async Task BackupThenRestoreIntoCleanContainer_ReVerifiesHashChainEndToEnd()
    {
        ...
        await using (var schemaContext = new ServiceHubDbContext(
            new DbContextOptionsBuilder<ServiceHubDbContext>().UseSqlite(sourceConnectionString).Options))
        {
        ...
        await using (var countContext = new ServiceHubDbContext(
            new DbContextOptionsBuilder<ServiceHubDbContext>().UseSqlite(sourceConnectionString).Options))
        {
        ...
        await using (var backupSourceContext = new ServiceHubDbContext(
            new DbContextOptionsBuilder<ServiceHubDbContext>().UseSqlite(sourceConnectionString).Options))
        {
        ...
        await using var restoredContext = new ServiceHubDbContext(
            new DbContextOptionsBuilder<ServiceHubDbContext>().UseSqlite(restoredConnectionString).Options);
    ...
    public async Task RestoredDatabase_TamperedAfterRestore_ChainVerificationDetectsIt()
    {
        ...
        await using var verifyContext = new ServiceHubDbContext(
            new DbContextOptionsBuilder<ServiceHubDbContext>().UseSqlite(restoredConnectionString).Options);
