...
// Ported from 4.0.0 (unit 5.7) under the owner-approved 2.6-style exception: 4.1.0's ServiceHubDbContext and IAuditTrail in the
// setup, and the two audit verifications name RecordAsync (4.1.0) instead of Enqueue (4.0.0) — same row, same action. Every
// other assertion is 4.0.0's.

public sealed class GovernanceGrantServiceTests : IDisposable
{
    private readonly ServiceHubDbContext _dbContext;
    private readonly Mock<IAuditTrail> _auditService = new();
    ...
    public async Task GrantAsync_ValidRequest_CreatesGrantAndWritesAuditLog()
    {
        ...
        _auditService.Verify(a => a.RecordAsync(It.Is<AuditLog>(l => l.Action == "Governance.Grant"), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task RevokeAsync_ActiveGrant_SetsRevokedAtAndWritesAuditLog_NeverDeletesRow()
    {
        ...
        _auditService.Verify(a => a.RecordAsync(It.Is<AuditLog>(l => l.Action == "Governance.Revoke"), It.IsAny<CancellationToken>()), Times.Once);
    }
