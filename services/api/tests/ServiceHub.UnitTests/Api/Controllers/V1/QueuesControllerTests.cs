...
public class QueuesControllerTests
{
    ...
    private static CloudEntity CreateTestEntity(string name, long active, long dlq)
    {
    ...
    [Fact]
    public async Task GetAll_AwsNamespace_ShouldListQueuesViaProvider()
    {
        ...
        awsProvider.Setup(p => p.ListEntitiesAsync(ns.Id, It.IsAny<CancellationToken>()))
            ...
            [
                ...
                new CloudEntity { Name = "orders-topic", EntityType = "SNS Topic", Provider = CloudProviderType.Aws },
            ]));
        ...
        var okResult = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        var queues = okResult.Value.Should().BeAssignableTo<IReadOnlyList<QueueRuntimePropertiesDto>>().Subject;
        queues.Should().ContainSingle();
        queues[0].Name.Should().Be("orders");
        queues[0].ActiveMessageCount.Should().Be(5);
        queues[0].DeadLetterMessageCount.Should().Be(2);
    ...
    public async Task GetMessages_ActiveMessages_ShouldReturnOk()
    {
        ...
        _cloudProvider.Setup(p => p.ListEntitiesAsync(ns.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<IReadOnlyList<CloudEntity>>.Success(
                [CreateTestEntity("test-queue", active: 10, dlq: 2)]));
