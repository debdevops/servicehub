...
using ServiceHub.UnitTests.RecoveryLedger;
using ServiceHub.Core.Results;

namespace ServiceHub.UnitTests.Agents;

// Ported from 4.0.0's AutonomyEvaluationWorkerTests under the owner-approved 2.6-style exception (2026-09-26). Changed:
// setup only — 4.1.0's ServiceHubDbContext, TrustTestLedger for the flag writes, the worker is now an agent (so the
// event bus is found in the sweep's scope, not the root), and the circuit-breaker section is dropped (unit 3.6).
// Every remaining assertion is 4.0.0's.

/// <summary>
/// Phase D coverage for <see cref="AutonomyEvaluationAgent"/>: the sweep finds every signature
...
public sealed class AutonomyEvaluationAgentTests : IDisposable
{
    ...
    public AutonomyEvaluationAgentTests()
    {
        ...
        _flags = new TrustTestLedger(_dbContext);
        _recoveryLedger = _flags.Real;
    ...
    private AutonomyEvaluationAgent CreateWorker(
        IDictionary<string, string?>? config = null, IPlatformEventBus? eventBus = null)
    {
        ...
        return new(
            ...
            NullLogger<AutonomyEvaluationAgent>.Instance);
    }
    ...
    private IServiceProvider BuildScope()
    {
        ...
        services.AddSingleton(_eventBus ?? Mock.Of<IPlatformEventBus>());
    ...
    public async Task SweepOwnerAsync_MeetsL4ButUnsafeFlagPresent_DoesNotPromote()
    {
        ...
        var flagResult = await _flags.RecordOutcomeFlagAsync(
    ...
    {
        AutonomyEvaluationAgent.DetermineTransition(AutonomyLevel.Approve, Evidence(3, 1.0, meetsL4: false, meetsL5: false), canProveDlqAbsence: true)
            .Should().BeNull();
    }
    ...
    {
        var transition = AutonomyEvaluationAgent.DetermineTransition(
    ...
    {
        AutonomyEvaluationAgent.DetermineTransition(
    ...
    {
        AutonomyEvaluationAgent.DetermineTransition(AutonomyLevel.Approve, Evidence(0, null, meetsL4: false, meetsL5: false), canProveDlqAbsence: true)
            .Should().BeNull();
    }
    ...
    // (4.0.0's SweepAutoReplayCircuitBreakersAsync tests are not ported: 4.1.0's rules own their breaker — unit 3.6.)
