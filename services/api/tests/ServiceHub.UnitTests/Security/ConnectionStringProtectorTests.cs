...
public sealed class ConnectionStringProtectorTests
{
    ...
    {
        var result = ServiceHub.Core.Results.Result.Success(42);
    ...
    {
        var result = ServiceHub.Core.Results.Result.Failure<int>(ServiceHub.Core.Results.Error.Validation("E", "err"));
    ...
    {
        var result = ServiceHub.Core.Results.Result.Success(5);
    ...
    {
        var err = ServiceHub.Core.Results.Error.Validation("E", "err");
        var result = ServiceHub.Core.Results.Result.Failure<int>(err);
        var mapped = result.Map(x => x * 10);
    ...
    {
        var result = ServiceHub.Core.Results.Result.Success(5);
        var bound = result.Bind(x => ServiceHub.Core.Results.Result.Success(x.ToString()));
    ...
    {
        var result = ServiceHub.Core.Results.Result.Success(5);
        var bound = result.Bind(x => ServiceHub.Core.Results.Result.Success());
        bound.IsSuccess.Should().BeTrue();
    }
    ...
    {
        var result = ServiceHub.Core.Results.Result.Success(10);
        var captured = 0;
    ...
    public void ResultT_Switch_OnFailure_ExecutesOnFailureAction()
    {
        var err = ServiceHub.Core.Results.Error.Validation("E", "err");
        var result = ServiceHub.Core.Results.Result.Failure<int>(err);
        ServiceHub.Core.Results.Error? captured = null;
    ...
    {
        var result = ServiceHub.Core.Results.Result<string>.Create("hello",
            ServiceHub.Core.Results.Error.Validation("E", "err"));
        result.IsSuccess.Should().BeTrue();
    ...
    {
        var err = ServiceHub.Core.Results.Error.Validation("E", "null");
        var result = ServiceHub.Core.Results.Result<string>.Create(null, err);
        result.IsFailure.Should().BeTrue();
    ...
    {
        var result = ServiceHub.Core.Results.Result.Success();
        var executed = false;
    ...
    {
        var err = ServiceHub.Core.Results.Error.Validation("E", "err");
        var result = ServiceHub.Core.Results.Result.Failure(err);
        ServiceHub.Core.Results.Error? captured = null;
    ...
    // ── Multi-key registry / ENC[v2:kid=...] envelope ─────────────────
