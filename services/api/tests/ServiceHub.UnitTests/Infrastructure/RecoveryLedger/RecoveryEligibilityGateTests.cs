...
public sealed class RecoveryEligibilityGateTests : IDisposable
{
    ...
    // ── Predicate 3: recurrence cap (§7.5) ────────────────────────────────────

    [Fact]
    public async Task FewerThanThreePriorMatches_NotBlocked()
    {
    ...
    [Fact]
    public async Task ThreePriorExactConfidenceMatches_EscalatesWithMatchedCount()
    {
        for (var i = 0; i < 3; i++)
    ...
    [Fact]
    public async Task ThreePriorHeuristicConfidenceMatches_EscalatesWithHeuristicReason()
    {
        for (var i = 0; i < 3; i++)
    ...
    [Fact]
    public async Task ThreePriorMatchesWithDivergentSignaturesAndUncorroboratedEntry_EscalatesAsAmbiguousCollision()
    {
    ...
    [Fact]
    public async Task RecurrenceCapAndRateLimitBothFire_Predicate3FiresBeforePredicate4()
    {
        for (var i = 0; i < 3; i++)
            SeedLineageEntry("orders", "hash-1", DateTimeOffset.UtcNow.AddDays(-i - 1), confidence: VerificationConfidence.Exact);
