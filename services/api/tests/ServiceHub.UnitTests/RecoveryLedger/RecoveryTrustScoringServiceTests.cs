...
// Ported from 4.0.0 under the owner-approved 2.6-style exception (2026-09-26): only the setup lines below changed —
// 4.1.0's ServiceHubDbContext, and TrustTestLedger supplying the declined/flag writes 4.1.0 does not ship. Every
// assertion is 4.0.0's.
...
public sealed class RecoveryTrustScoringServiceTests : IDisposable
{
    ...
    private readonly TrustTestLedger _ledger;
    ...
    public RecoveryTrustScoringServiceTests()
    {
        ...
        _ledger = new TrustTestLedger(_dbContext);
        _service = new RecoveryTrustScoringService(_ledger.Real);
    }
