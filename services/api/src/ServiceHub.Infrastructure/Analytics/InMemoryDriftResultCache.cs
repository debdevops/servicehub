...
/// <summary>
/// Process-local, TTL-bounded implementation of <see cref="IDriftResultCache"/>.
/// </summary>
public sealed class InMemoryDriftResultCache : IDriftResultCache
{
    private static readonly TimeSpan EntryLifetime = TimeSpan.FromHours(24);

    private readonly ConcurrentDictionary<Guid, (DriftFinding Finding, DateTimeOffset ExpiresAt)> _entries = new();

    /// <inheritdoc />
    public void Store(IEnumerable<DriftFinding> findings)
    {
    ...
    /// <inheritdoc />
    public DriftFinding? TryGet(Guid id)
    {
