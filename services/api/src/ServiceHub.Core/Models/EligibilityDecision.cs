...
/// when appropriate. <see cref="ReasonCode"/> is <see langword="null"/> only when
/// <see cref="Verdict"/> is <see cref="EligibilityVerdict.Allow"/>.
/// </summary>
/// <param name="Verdict">The gate's decision.</param>
/// <param name="ReasonCode">Which predicate produced a non-<c>Allow</c> verdict, e.g.
/// <c>"PURGE_AUTOMATION_PROHIBITED"</c>, <c>"RECURRENCE_CAP_EXCEEDED"</c>. Distinguishes
/// predicate 4 (rate limit — routine throttling, never <c>Declined</c>-recorded per §9.3) from
/// predicates 1/2/3 (real safety escalations, always <c>Declined</c>-recorded when they fire).</param>
