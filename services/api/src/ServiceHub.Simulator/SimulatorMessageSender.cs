...
public sealed class SimulatorMessageSender : IMessageSender
{
    ...
    public Task<Result> SendAsync(
        ...
        => Resolve(request.NamespaceId ?? Guid.Empty).SendAsync(request, cancellationToken);
        ...
        IEnumerable<SendMessageRequest> requests, CancellationToken cancellationToken = default)
    {
        ...
        return Resolve(list[0].NamespaceId ?? Guid.Empty).SendBatchAsync(list, cancellationToken);
    }
