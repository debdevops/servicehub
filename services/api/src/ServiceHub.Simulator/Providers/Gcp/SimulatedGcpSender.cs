...
/// <summary>
/// Simulated GCP Pub/Sub message sender backed by <see cref="ISimulatorStore"/>.
/// </summary>
public sealed class SimulatedGcpSender : IMessageSender
{
    private readonly ISimulatorStore _store;

    /// <summary>Initializes a new instance of <see cref="SimulatedGcpSender"/>.</summary>
    public SimulatedGcpSender(ISimulatorStore store)
    {
    ...
    public Task<Result> SendAsync(SendMessageRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (entity is null)
            return Task.FromResult(Result.Failure(
                Error.NotFound("Simulator.EntityNotFound",
                    $"Topic/subscription '{request.EntityName}' not found in namespace {request.NamespaceId}")));
        ...
        var msg = new SimulatorMessage(
            ...
            OrderingKey: request.PartitionKey,
            DeliveryAttempt: 0,
            AckDeadline: null,
            IsNacked: false,
            Provider: Core.Enums.CloudProviderType.Gcp);
