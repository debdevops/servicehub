...
import { fleetApi, type FleetOverview } from '../lib/api/fleet';
import { useDemoContext } from '../lib/demo/DemoContext';
import { getMockFleetOverview } from '../lib/demo/mockProviders';
...
export function useFleetOverview(windowHours = 24, enabled = true) {
  ...
  const options: UseQueryOptions<FleetOverview> =
    isDemoMode && cloudProvider
      ? {
          queryKey: ['fleet-overview', 'demo', cloudProvider],
          queryFn: (): Promise<FleetOverview> => Promise.resolve(getMockFleetOverview(cloudProvider)),
        }
      : {
