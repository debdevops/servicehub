 ...
 */
export function RecentDeadLetters({ provider, namespaces }: { provider: CloudProvider; namespaces: readonly Namespace[] }) {
  const { data } = useDeadLetters({ provider, status: 'active', page: 1, pageSize: 5 })
  ...
  return (
    <section aria-label="Latest dead letters" className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]">
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <Link to="/?tab=dlq" className="text-sm font-medium text-[var(--color-primary-700)] hover:underline">
          See all {data.paging.total.toLocaleString()} →
        </Link>
      </div>
      <MessageTable
        rows={data.items}
        namespaceNames={new Map(namespaces.map((n) => [n.id, n.displayName ?? n.name]))}
        compact
