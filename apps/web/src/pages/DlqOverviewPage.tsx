...
import { getProviderStyle, getProviderServiceName } from '@servicehub/ui-shared/lib/providerStyles';
...
function windowLabel(days: number): string {
  return days === 1 ? 'vs last 24 hours' : `vs last ${days} days`;
}
...
// Real per-provider identifiers only — no fabricated hostnames. AWS/GCP namespaces carry a
// region/project id; Azure namespaces don't expose an equivalent field today, so it falls back to
// the service name alone.
function providerSubtitle(provider: CloudProviderType, namespaces: Namespace[] | undefined): string {
  const match = namespaces?.find((ns) => ns.cloudProvider === provider && (ns.awsRegion || ns.gcpProjectId));
  const detail = provider === 'aws' ? match?.awsRegion : provider === 'gcp' ? match?.gcpProjectId : undefined;
  return detail ? `${getProviderServiceName(provider)} · ${detail}` : getProviderServiceName(provider);
}
...
function ChangeBadge({
  ...
  return (
    <span
      title={title}
      className={`inline-flex items-center gap-0.5 text-[11px] font-semibold ${isGood ? 'text-emerald-600' : 'text-red-600'}`}
    >
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        {caption && <div className="text-[11px] text-gray-400 truncate">{caption}</div>}
      </div>
    </div>
  );
}

// ─── Row action menu (kebab) — mirrors MessagesOverviewPage's RowMenu ─────────────────────────
...
function NamespaceTableRow({ ns, onView }: { ns: DlqOverviewNamespace; onView: () => void }) {
  const [menuOpen, setMenuOpen] = useState<DOMRect | null>(null);
  return (
    <tr className="border-t border-gray-100 hover:bg-gray-50">
      <td className="px-3 py-2 font-medium text-gray-900 truncate max-w-[160px]" title={ns.namespaceName}>
        {ns.namespaceName}
      </td>
      ...
      <td className="px-3 py-2 text-gray-500 whitespace-nowrap">{formatAge(ns.oldestDetectedAt)}</td>
      <td className="px-3 py-2">
        <div className="flex items-center justify-end gap-1">
          <button
            onClick={onView}
            className="px-2.5 py-1 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-md transition-colors whitespace-nowrap"
          >
            View
          </button>
          <button
            onClick={(e) => setMenuOpen((e.currentTarget as HTMLElement).getBoundingClientRect())}
            aria-label={`More actions for ${ns.namespaceName}`}
              ...
              actions={[
                {
                  ...
                  onClick: () => navigator.clipboard?.writeText(ns.namespaceName),
                },
              ]}
            />
          )}
...
function ProviderSection({
  ...
  return (
    <section className={`bg-white rounded-xl border ${style.headerBorder} shadow-sm overflow-hidden`}>
      <div className={`flex flex-wrap items-center gap-4 px-5 py-4 ${style.headerBg} border-b ${style.headerBorder}`}>
        <button
          onClick={() => setCollapsed((v) => !v)}
          aria-expanded={isOpen}
          className="flex items-center gap-3 min-w-0 flex-1 text-left"
        >
          <ChevronDown className={`w-4 h-4 text-gray-400 shrink-0 transition-transform ${isOpen ? '' : '-rotate-90'}`} />
          <div className={`w-10 h-10 rounded-lg bg-white border ${style.headerBorder} flex items-center justify-center shrink-0`}>
            <ProviderIcon provider={section.provider} className="w-5 h-5" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-base font-bold text-gray-900">{style.label}</h2>
              <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-white/70 text-gray-500 border border-gray-200 whitespace-nowrap">
        ...
        <div className="flex items-center gap-5 shrink-0">
          <div className="text-right">
            <div className="flex items-center gap-1.5 justify-end">
              <span className={`text-lg font-bold ${section.totalDeadLettered > 0 ? 'text-red-700' : 'text-gray-400'}`}>
          ...
          <div className="text-right hidden sm:block">
            <div className="flex items-center gap-1 justify-end text-gray-700">
              ...
              <span className="text-sm font-semibold">{section.affectedQueues}</span>
            </div>
            <div className="text-[10px] text-gray-500 -mt-0.5">Queues</div>
          </div>
          <div className="text-right hidden sm:block">
            <div className="flex items-center gap-1 justify-end text-gray-700">
              <Radio className="w-3.5 h-3.5 text-gray-400" />
              <span className="text-sm font-semibold">{section.affectedTopics}</span>
            </div>
            <div className="text-[10px] text-gray-500 -mt-0.5">Topics</div>
          </div>
          <button
            onClick={onViewAll}
            className="px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors whitespace-nowrap"
          >
            View All
          </button>
        </div>
      ...
      {isOpen && (
        <div className="p-4">
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
            <div className="xl:col-span-4 border border-gray-100 rounded-lg p-3">
              <div className="flex items-center justify-between gap-2 mb-2">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">DLQ Trend ({style.label})</h3>
                <ChangeBadge percent={section.changePercent} title={windowLabel(windowDays)} />
              </div>
                ...
                <div className="space-y-2">
                  {topReasons.map((reason, i) => (
                    <div key={reason.category} className="flex items-center gap-2 text-xs">
                      <span className="w-20 shrink-0 text-gray-600 truncate">{REASON_LABELS[reason.category]}</span>
                      ...
                      <span className="w-8 shrink-0 text-right font-semibold text-gray-700">{reason.count}</span>
                      <span className="w-9 shrink-0 text-right text-gray-400">{reason.percent}%</span>
                    </div>
                  ))}
                </div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {visibleNamespaces.map((ns) => (
                        <NamespaceTableRow key={ns.namespaceId} ns={ns} onView={() => onNavigateNamespace(ns)} />
                      ))}
                    </tbody>
                  </table>
                </div>
...
export default function DlqOverviewPage() {
  ...
  const goToMessagesOverview = (tab: 'active' | 'deadletter', provider?: CloudProviderType) =>
    navigate(`${navPrefix}/messages-overview?tab=${tab}${provider ? `&cloud=${provider}` : ''}`);
    ...
    <div className="flex-1 overflow-y-auto min-w-0">
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <p className="text-xs text-gray-400">
          <button onClick={() => navigate('/fleet')} className="hover:text-gray-600 hover:underline">
            Fleet Overview
          </button>
          {' / '}
          <button onClick={() => goToMessagesOverview('active')} className="hover:text-gray-600 hover:underline">
            Messages
          </button>
          {' / '}
          <span className="text-gray-500 font-medium">Dead-Letter</span>
        </p>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex rounded-lg overflow-hidden border border-gray-300 shrink-0">
              <button
                onClick={() => goToMessagesOverview('active')}
                className="px-4 py-1.5 text-sm font-medium bg-white text-gray-600 hover:bg-gray-50 transition-colors"
              >
                Active
              </button>
              <button aria-current="page" className="px-4 py-1.5 text-sm font-medium bg-red-600 text-white">
                Dead-Letter
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              {data && (
                <span className="text-xs text-gray-400 hidden sm:inline">Last updated: {formatTimestamp(data.generatedAt)}</span>
              )}
              <button
                onClick={() => refetch()}
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              <StatTile
                ...
                tone="bg-red-50"
                sub={<ChangeBadge percent={data.totals.changePercent} title={windowLabel(data.windowDays)} />}
              />
              <StatTile
                ...
                tone="bg-sky-50"
                sub={<ChangeBadge percent={data.totals.namespacesWithDlqChangePercent} title={windowLabel(data.windowDays)} />}
              />
              <StatTile
                icon={<Layers className="w-5 h-5 text-purple-600" />}
                label="Affected Queues"
                value={data.totals.affectedQueues.toLocaleString()}
                tone="bg-purple-50"
                sub={<ChangeBadge percent={data.totals.affectedQueuesChangePercent} title={windowLabel(data.windowDays)} />}
              />
              <StatTile
                icon={<Radio className="w-5 h-5 text-violet-600" />}
                label="Affected Topics"
                value={data.totals.affectedTopics.toLocaleString()}
                tone="bg-violet-50"
                sub={<ChangeBadge percent={data.totals.affectedTopicsChangePercent} title={windowLabel(data.windowDays)} />}
              />
              <StatTile
                ...
                tone="bg-gray-100"
                caption={formatTimestamp(data.totals.oldestMessageDetectedAt)}
              />
            </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-3 flex flex-wrap items-center gap-2">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <button
                    ...
                    title="More filters — entity name, status, replay safety"
                    className={`p-2 rounded-lg border transition-colors ${
                      showMoreFilters ? 'bg-red-50 border-red-300 text-red-700' : 'border-gray-300 text-gray-600 bg-white hover:bg-gray-50'
                    }`}
                  >
              ...
              <div className="space-y-4">
                {data.providers.map((section) => (
                  <ProviderSection
                    ...
                    section={section}
                    subtitle={providerSubtitle(section.provider, namespaces)}
                    windowDays={data.windowDays}
                    search={search}
                    onNavigateNamespace={goToNamespace}
                    onViewAll={() => goToMessagesOverview('deadletter', section.provider)}
                  />
                ))}
              </div>
            )}
